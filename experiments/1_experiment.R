...
weights <- init_weights(dim_r = dim_r, dim_h_hidden = 16L, dim_g_hidden = 16L)
...
# set up NN
train_op_and_loss <- init_NP(weights, x_context, y_context, x_target, y_target)
# initialise
init <- tf$global_variables_initializer()
sess$run(init)

# Plotting functionality, using fixed source of randomness over all iters
n_draws <- 50L
...
predict_op <- posterior_predict(weights, cbind(x), cbind(y), cbind(x_star), epsilon)
...
p <- df_pred %>%
  ggplot(aes(x, y)) +
  geom_line(aes(group=rep_index), alpha = 0.3) +
  geom_point(data = df_obs_rep, col = "#b2182b") +
