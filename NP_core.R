...
init_weights <- function(dim_r = 8L, dim_z = dim_r, dim_h_hidden = 16L, dim_g_hidden = 16L){
  # weights for the encoder h, mapping (x, y) -> hidden -> r
  W_h1 <- tf$Variable(tf$random_normal(shape(2L, dim_h_hidden)))
  b_h1 <- tf$Variable(tf$random_normal(shape(dim_h_hidden)))
  W_h2 <- tf$Variable(tf$random_normal(shape(dim_h_hidden, dim_r)))
  b_h2 <- tf$Variable(tf$random_normal(shape(dim_r)))
  ...
  b_g1 <- tf$Variable(tf$random_normal(shape(dim_g_hidden)))
  W_g2 <- tf$Variable(tf$random_normal(shape(dim_g_hidden, 1L)))
  b_g2 <- tf$Variable(tf$random_normal(shape(1L)))
...
map_xy_to_z_params <- function(x, y, weights){
  list(x, y) %>%
    tf$concat(axis = 1L) %>%
    h(weights$W_h1, weights$b_h1, weights$W_h2, weights$b_h2) %>%
    aggregate_r() %>%
    get_z_params(weights$W_z_mu, weights$W_z_sigma)
}

# set up the NN architecture with train_op and loss
init_NP <- function(weights, x_context, y_context, x_target, y_target, learning_rate = 0.001){
  ...
  # map (z, x*) to y*
  y_pred_params <- g(z_sample, x_target, weights$W_g1, weights$b_g1, weights$W_g2, weights$b_g2)
...
prior_predict <- function(weights, x_star_value, epsilon = NULL, n_draws = 1L){
  ...
  # y ~ g(z, x*)
  y_star <- g(z_sample, x_star, weights$W_g1, weights$b_g1, weights$W_g2, weights$b_g2)
